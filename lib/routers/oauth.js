...
router.get('/callback', async (ctx, next) => {
  ...
  // 获取openid
  let meData = await ctx.trpgapp.request.get(GET_USER_INFO_URL, {access_token});
  ...
  userData = JSON.parse(userData.replace(/callback|\(|\)|;/g, ''))
  let {
    gender,
    nickname,
    figureurl,
  ...
  await db.transactionAsync(async () => {
    ...
    if(record) {
      ...
      let player = await db.models.player_user.getAsync(playerId);
      player.token = uuid();
      ...
      newRecord.related_user_id = newPlayer.id;
